<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="链接Logo图片地址" prop="logoUrl">
        <el-input
          v-model="queryParams.logoUrl"
          placeholder="请输入链接Logo图片地址"
          clearable
          @keyup.enter.native="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="linkList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="友情链接唯一标识，自增主键" align="center" prop="id" />
      <el-table-column label="链接名称" align="center" prop="linkName" />
      <el-table-column label="链接地址" align="center" prop="linkUrl" />
      <el-table-column label="链接Logo图片地址" align="center" prop="logoUrl" />
      <el-table-column label="显示顺序" align="center" prop="displayOrder" />
      <el-table-column label="链接状态：1-显示，0-隐藏" align="center" prop="status" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="链接Logo图片地址" prop="logoUrl">
          <el-input v-model="form.logoUrl" placeholder="请输入链接Logo图片地址" />
        </el-form-item>
        <el-form-item label="显示顺序" prop="displayOrder">
...
export default {
  ...
  methods: {
    ...
    handleAdd() {
      this.reset()
      this.open = true
      this.title = "添加首页友情链接，管理首页展示的外部链接"
    },
    /** 修改按钮操作 */
    handleUpdate(row) {
      this.reset()
      ...
      getLink(id).then(response => {
        ...
        this.open = true
        this.title = "修改首页友情链接，管理首页展示的外部链接"
      })
    },
    /** 提交按钮 */
    submitForm() {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除首页友情链接，管理首页展示的外部链接编号为"' + ids + '"的数据项？').then(function() {
        return delLink(ids)
      }).then(() => {
        this.getList()
